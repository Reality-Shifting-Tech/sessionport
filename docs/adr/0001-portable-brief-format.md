# ADR-0001: Portable brief format (sessionport-brief/v1)
...
sessionport exists to move agent context between different agent CLIs whose native
...
frontmatter (format id `sessionport-brief/v1`, source agent, session id, export
...
- Extraction is deterministic and offline (see ADR-0003 for the optional LLM
  layer).
- The format is versioned in the frontmatter (`format: sessionport-brief/v1`) so
